...
function setDockSelection(nextSelection) {
  dockSelection = nextSelection;
...
function createDockCard(item) {
  ...
  const amount = checklist.amount ? fmtCurr0(Number(entry.amount || entry.budget || 0)) : '—';
...
export async function createManualDeal(payload) {
  if (isUnsavedChangesBlocker()) {
    showToast('Bitte erst speichern oder Änderungen verwerfen.', 'warn');
    return;
  }

  const url = `${WORKER_BASE}/entries`;
  const response = await fetchWithRetry(url, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(payload || {}),
  ...
  if (!response.ok) {
    ...
    throw new Error(`Manueller Deal konnte nicht erstellt werden (${response.status}) ${detail}`.trim());
  }

  const created = await response.json();
  showToast('Manueller Deal erstellt.', 'ok');
  renderPortfolio();
  await renderDockBoard();
  return created;
}

export function openDockEntryDialog() {
  if (!dockEntryDialog) return;
  if (typeof dockEntryDialog.showModal === 'function') {
